...
#include "proc.h"
#include "defs.h"
#define RAND_MAX 32768  // Max value for rand()
#define RAND_A 1103515245  // Multiplier
#define RAND_C 12345       // Increment
#define RAND_M RAND_MAX    // Modulus (2^15)
...
struct proc *p;
  struct cpu *c = mycpu();
  for(;;){
    // Enable interrupts and allow the CPU to sleep
    intr_on();
    int total = 0;
    ...
    while (p < &proc[NPROC]) {
      if(p->state == RUNNABLE) {
        if(p->tickets == selected->tickets && p->start_time < selected->start_time) {
          selected = p;
          flag = 1;
        }
      }
    ...
    if(flag) {
      ...
      c->proc = selected;
      // switch(&c->context)
      swtch(&c->context, &selected->context); // Switch to the selected process's context
...
void scheduler(void) {
 ...
 for(;;)
  ...
  int priorty_boost_flag = ticks%48;
